...
public class UbigeoImpl extends Conexion implements UbigeoI {
    ...
    public List<String> autocompleteUbigeo(String a) throws Exception {
        ...
        List<String> Lista;
        try {
            String sql = "SELECT UPPER(DEPUBI ||', '|| PROUBI ||', '|| DISUBI) AS UBI FROM UBIGEO WHERE UPPER(DEPUBI ||', '|| PROUBI ||', '|| DISUBI) LIKE UPPER(?)";
    ...
    public String obtenerCodigoUbigeo(String a) throws Exception {
        ...
        ResultSet rs;
        try {
            String sql = "SELECT CODUBI FROM UBIGEO WHERE UPPER(DEPUBI ||', '|| PROUBI ||', '|| DISUBI) = ?";
            PreparedStatement ps = this.getCn().prepareCall(sql);
            ps.setString(1, a);
            rs = ps.executeQuery();
            if (rs.next()) {
